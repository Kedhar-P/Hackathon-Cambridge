...
//            NotificationCenter.default.post(name: NSNotification.Name(rawValue: "securityOnPinOrDrop"), object: nil, userInfo: ["percentage":CGFloat(alpha)])
        ...
        if swipeDown == true {
            ...
            if viewPinned {
                ...
                if view == views[0] {
                    ...
                    if globalVars.pinnedViews[0] == 2 {
                        unPinSecurityDeposit()
                        unPinComplaints()
                        changeViewOnPinOrDrop(show: false, viewToChange: views[2])
                    } else if globalVars.pinnedViews[0] == 1 {
                        unPinComplaints()
                        changeViewOnPinOrDrop(show: false, viewToChange: views[1])
                    } else {
                        changeViewOnPinOrDrop(show: false, viewToChange: views[0])
                    }
                    unPinPayment()
                    viewPinned = false
                    ...
                    if globalVars.pinnedViews[0] == 2 {
                        unPinSecurityDeposit()
                        unPinComplaints()
                        changeViewOnPinOrDrop(show: true, viewToChange: views[0])
                        changeViewOnPinOrDrop(show: false, viewToChange: views[2])
                    } else if globalVars.pinnedViews[0] == 1 {
                        unPinComplaints()
                        changeViewOnPinOrDrop(show: true, viewToChange: views[0])
                ...
                } else if globalVars.pinnedViews[0] == 2 {
                    unPinSecurityDeposit()
                    changeViewOnPinOrDrop(show: true, viewToChange: views[1])
                    changeViewOnPinOrDrop(show: false, viewToChange: views[2])
                    //TODO: If dragged down but never pinned then reset y position to original
                }
                
                //viewPinned = false
                //changeViewOnPinOrDrop(alpha: 1)
            ...
            var snapPosition = self.view.center
            snapPosition.y += 152 //TODO: Vary if screen size different?????
            
            if !viewPinned { // If swiped up and no view is pinned then snap this one
                ...
                if view == views[0] {
                    ...
                    pinSecurityDeposit()
                    changeViewOnPinOrDrop(show: true, viewToChange: views[2])
                }
                viewPinned = true

            } else { //If swiped up and a view is pinned then unsap then snap new

                // 1) unpin current pinned view
                if globalVars.pinnedViews[0] == 0 {
                    unPinPayment()
                } else if globalVars.pinnedViews[0] == 1 {
                    print("unpin complaints")
                    unPinPayment()
                    unPinComplaints()
                    print("unpin complaints")
                } else {
                ...
                viewPinned = true
               // changeViewOnPinOrDrop(alpha: 0) // TODO: has to be specific for the view that is being unpinned
    ...
    func pinPayment() {
        var snapPosition = self.view.center //Make global????
        snapPosition.y += 152 //TODO: Vary if screen size different?????
        paymentSnap = UISnapBehavior(item: views[0], snapTo: snapPosition)
        animator.addBehavior(paymentSnap)
        animator.updateItem(usingCurrentState: views[0])
        globalVars.pinnedViews[0] = 0
        print(globalVars.pinnedViews[0])
    }
    
    func unPinPayment() {
        animator.removeBehavior(paymentSnap)
        animator.updateItem(usingCurrentState: views[0])
        globalVars.pinnedViews[0] = 9
        print(globalVars.pinnedViews[0])
    }
    ...
    func pinComplaints() {
        pinPayment()
        var snapPosition = self.view.center //Make global????
        snapPosition.y += 152 + 115 //TODO: Vary if screen size different?????
        complaintSnap = UISnapBehavior(item: views[1], snapTo: snapPosition)
        animator.addBehavior(complaintSnap)
        animator.updateItem(usingCurrentState: views[1])
        globalVars.pinnedViews[0] = 1
        print(globalVars.pinnedViews[0])
    }
    
    func unPinComplaints() {
        //unPinPayment()
        animator.removeBehavior(complaintSnap)
        animator.updateItem(usingCurrentState: views[1])
        globalVars.pinnedViews[0] = 0
        print(globalVars.pinnedViews[0])
    }
    
    func pinSecurityDeposit() {
        pinComplaints()
        var snapPosition = self.view.center //Make global????
        snapPosition.y += 152 + 115 + 115 //TODO: Vary if screen size different????? //TODO: make 115 global var???
        securityDeposit = UISnapBehavior(item: views[2], snapTo: snapPosition)
        animator.addBehavior(securityDeposit)
        animator.updateItem(usingCurrentState: views[2])
        globalVars.pinnedViews[0] = 2
        print(globalVars.pinnedViews[0])
    }
    
    func unPinSecurityDeposit() {
        
        //unPinComplaints()
        animator.removeBehavior(securityDeposit)
        animator.updateItem(usingCurrentState: views[2])
        globalVars.pinnedViews[0] = 1
        print(globalVars.pinnedViews[0])
    }
